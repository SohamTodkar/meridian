...
export function importBackup(
  ...
  const isMeridian =
    (input.version === 1 ||
      input.version === 2 ||
      (input.app === "Meridian" &&
        input.format === "meridian-backup" &&
        input.schemaVersion === 2)) &&
    isRecord(input.state);
  const wrapped = isMeridian && isRecord(input.state) ? input.state : input;
  if (
    isMeridian &&
    isRecord(wrapped.checks) &&
    isRecord(wrapped.sessions) &&
    isRecord(wrapped.settings)
  ) {
    return {
      source: "Meridian",
      matched:
        Object.keys(wrapped.checks).length +
        Object.keys(wrapped.sessions).length,
      skipped: 0,
      state: wrapped as Partial<MeridianStateSnapshot>,
    };
  }
  ...
  const sessionIds = new Set(
    model.phases.flatMap(phase => phase.sessions.map(session => session.id))
  );
  ...
  const journals = Array.isArray(wrapped.journal)
    ? wrapped.journal
        .filter(isRecord)
        .map((entry, index) => ({
  ...
  return {
    ...
    state: {
      ...
      settings: {
        scheduleMode:
          typeof wrapped.scheduleMode === "string"
            ? wrapped.scheduleMode
            : typeof wrapped.mode === "string"
              ? wrapped.mode === "flex"
                ? "flexible"
                : wrapped.mode
              : "normal",
