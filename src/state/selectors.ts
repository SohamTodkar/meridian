...
export type PlanTaskStatus =
  "proposed" | "accepted" | "completed" | "deferred" | "dismissed";
...
export function weightedCapabilityProgress(
  ...
  const weightedCompleted =
    completedSessions * CAPABILITY_WEIGHTS.session +
    completedEvidence * CAPABILITY_WEIGHTS.phaseEvidence;
  const weightedTotal =
    sessions * CAPABILITY_WEIGHTS.session +
    evidence * CAPABILITY_WEIGHTS.phaseEvidence;
  return {
    ...progressValue(
      completedSessions + completedEvidence,
      sessions + evidence
    ),
    weightedCompleted,
    weightedTotal,
...
export function getMeasuredMinutes(log?: DailyLog): number {
  ...
  return Math.round(
    log.timeEntries.reduce((sum, entry) => sum + entry.seconds, 0) / 60
  );
}
...
export function getLearningMinutes(log?: DailyLog): number {
  ...
  const manual =
    log.manualMinutes ??
    (log.timeEntries?.length
      ? Math.max(0, (log.minutes ?? 0) - measured)
      : (log.minutes ?? 0));
...
export function phaseEvidenceIds(phase: Phase): string[] {
  return [
    ...phase.checkpoint.cockpit.requirements.map(
      (_, index) => `${phase.id}.checkpoint.requirement.${index + 1}`
    ),
    ...phase.checkpoint.northstar.map(
      (_, index) => `${phase.id}.gate.${index + 1}`
    ),
  ];
}

export function sessionCheckIds(phase: Phase): string[] {
  return phase.sessions.flatMap(session =>
    session.checks.map((_, index) => `${session.id}.check.${index + 1}`)
  );
}

export function isPhaseCleared(
  phase: Phase,
  state: MeridianStateSnapshot
): boolean {
...
export function getPhaseAccess(
  ...
  if (phaseIndex <= 0)
    return { allowed: true, overridden: false, requirements: [] };
  if (state.progressionOverrides?.[phaseOverrideKey(phase.id)])
    return { allowed: true, overridden: true, requirements: [] };
  const prerequisite = model.phases[phaseIndex - 1];
  if (isPhaseCleared(prerequisite, state))
    return { allowed: true, overridden: false, requirements: [] };
  return {
    allowed: false,
    overridden: false,
    requirements: [
      {
        key: phaseOverrideKey(prerequisite.id),
        label: `Clear Phase ${prerequisite.identity.number + 1}: ${prerequisite.identity.northstarName} checkpoint evidence.`,
      },
    ],
  };
}
...
export function getSessionAccess(
  ...
  if (state.sessions[sessionId]?.completed)
    return { allowed: true, overridden: false, requirements: [] };
  if (state.progressionOverrides?.[sessionOverrideKey(sessionId)])
    return { allowed: true, overridden: true, requirements: [] };
  ...
  if (index <= 0 || !previous || state.sessions[previous.id]?.completed)
    return {
      allowed: true,
      overridden: phaseAccess.overridden,
      requirements: [],
    };
  return {
    allowed: false,
    ...
    requirements: [
      {
        key: sessionOverrideKey(previous.id),
        label: `Complete Session ${index}: ${previous.title}.`,
      },
    ],
  };
}
...
export function getPhaseProgress(
  ...
  void model;
  const curriculum = phase.curriculum.reduce(
    (sum, section) => sum + section.items.length,
    0
  );
  const sessions = phase.sessions.length;
  const phaseEvidence = phaseEvidenceIds(phase).length;
  const sessionChecks = phase.sessions.reduce(
    (sum, session) => sum + session.checks.length,
    0
  );
  const completedCurriculum = phase.curriculum.reduce(
    (sum, section) =>
      ...
      section.items.reduce(
        (itemSum, _, index) =>
          itemSum + (state.checks[`${section.key}.${index + 1}`] ? 1 : 0),
        0
      ),
    0
  );
  const completedSessions = phase.sessions.reduce(
    (sum, session) => sum + (state.sessions[session.id]?.completed ? 1 : 0),
    0
  );
  const completedEvidence = phaseEvidenceIds(phase).filter(
    id => state.checks[id]
  ).length;
  const completedSessionChecks = sessionCheckIds(phase).filter(
    id => state.checks[id]
  ).length;
  const coverage = progressValue(completedCurriculum, curriculum);
  const evidence = progressValue(
    completedSessionChecks + completedEvidence,
    sessionChecks + phaseEvidence
  );
  const capability = weightedCapabilityProgress(
    completedSessions,
    sessions,
    completedEvidence,
    phaseEvidence
...
export function getOverallProgress(
  ...
  const progress = model.phases.map(phase =>
    getPhaseProgress(model, phase, state)
  );
  const completed = progress.reduce((sum, item) => sum + item.completed, 0);
  const total = progress.reduce((sum, item) => sum + item.total, 0);
  const weightedCompleted = progress.reduce(
    (sum, item) => sum + item.weightedCompleted,
    0
  );
  const weightedTotal = progress.reduce(
    (sum, item) => sum + item.weightedTotal,
    0
  );
  const capability = {
...
export function getActivePhase(
  ...
  return (
    model.phases.find(phase => !isPhaseCleared(phase, state)) ??
    model.phases.at(-1)!
  );
}
...
export function getNextAction(
  model: MeridianModel,
  state: MeridianStateSnapshot
): { type: "session" | "checkpoint"; phase: Phase; sessionId?: string } {
  const phase = getActivePhase(model, state);
  const session = phase.sessions.find(
    item => !state.sessions[item.id]?.completed
  );
  return session
    ? { type: "session", phase, sessionId: session.id }
    : { type: "checkpoint", phase };
}
...
function planningMode(model: MeridianModel, state: MeridianStateSnapshot) {
  return model.modes.find(
    item =>
      item.category === "schedule" && item.key === state.settings.scheduleMode
  );
}
...
export function getDailyCapacity(
  ...
  state: MeridianStateSnapshot,
  date: string
): { minutes: number; isRestDay: boolean; mode: string; reason: string } {
  const mode = planningMode(model, state);
  const weeklyHours =
    state.settings.pace?.weeklyHours ??
    (mode?.category === "schedule" ? mode.weekly : undefined) ??
    0;
  const touchMinutes =
    state.settings.pace?.dailyTouchMinutes ??
    (mode?.category === "schedule" ? mode.touchMin : undefined) ??
    20;
  const isRestDay =
    weekdayForDateKey(date) === (state.settings.pace?.restDay ?? "Sunday");
  const minutes = isRestDay
    ? touchMinutes
    : Math.max(touchMinutes, Math.round((weeklyHours * 60) / 6));
  return {
    minutes,
    isRestDay,
    mode: mode?.label ?? "Personal pace",
    reason: isRestDay
      ? "Rest-day floor: keep the learning identity alive without using catch-up volume."
      : `Six-day allocation from the ${mode?.label ?? "personal"} weekly target.`,
  };
}

export function getDailyPlanRecommendation(
  model: MeridianModel,
  state: MeridianStateSnapshot,
  date: string
  ...
  const deferred = Object.entries(state.dailyPlans ?? {})
    ...
    .flatMap(([, plan]) =>
      plan.tasks.filter(
        task => task.status === "deferred" && task.deferredTo === date
      )
    );
  const reasons = [capacity.reason];
  const tasks: PlannedTask[] = deferred.map(task => ({
    ...task,
    status: "proposed",
    deferredTo: undefined,
    reason: `Carried forward from a deliberate deferral: ${task.reason}`,
  }));
  ...
  if (duePrompts.length) {
    duePrompts.forEach(prompt =>
      tasks.push({
        id: `retrieval:${prompt.id}`,
        kind: "retrieval",
        title: "Retrieve without notes",
        detail: prompt.question,
        minutes: Math.max(1, Math.floor(retrievalMinutes / duePrompts.length)),
        status: "proposed",
        reason: `Due retrieval from ${prompt.dueDate}. Answer first, then check your original context.`,
        retrievalPromptId: prompt.id,
      })
    );
    reasons.push(
      `${duePrompts.length} local retrieval prompt${duePrompts.length === 1 ? " is" : "s are"} due before new material.`
    );
    ...
    tasks.push({
      ...
      detail: "Reviews only. Keep retrieval alive.",
      minutes: retrievalMinutes,
      status: "proposed",
      reason: "Daily retrieval floor protects retention.",
    });
  }
  if (!capacity.isRestDay) {
    ...
    const available = Math.max(
      0,
      capacity.minutes - claimed - retrievalMinutes
    );
    if (next.type === "session" && next.sessionId && available > 0) {
      const session = next.phase.sessions.find(
        item => item.id === next.sessionId
      );
      if (session && !tasks.some(task => task.sessionId === session.id)) {
        tasks.push({
          ...
          kind: "session",
          title: session.title,
          detail: session.outcome,
          minutes: Math.min(session.minutes, available),
          status: "proposed",
          sessionId: session.id,
          reason: `Next unlocked session in Phase ${next.phase.identity.number + 1}; this is the smallest useful forward move.`,
        });
      }
      ...
      tasks.push({
        ...
        kind: "checkpoint",
        title: "Phase checkpoint evidence",
        detail:
          "Convert completed work into the proof that opens the next phase.",
        minutes: Math.max(10, available),
        status: "proposed",
        reason:
          "All active sessions are complete; checkpoint evidence is the blocking next action.",
      });
    }
  }
  if (!tasks.some(task => task.id === "reflection:daily")) {
    tasks.push({
      id: "reflection:daily",
      kind: "reflection",
      title: "Daily learning capture",
      detail: "Write one honest line: evidence, confusion, or next question.",
      minutes: 5,
      status: "proposed",
      reason: "A short record preserves the signal for the weekly review.",
    });
  }
  return {
    date,
...
export function getWeeklyPlanSummary(
  state: MeridianStateSnapshot,
  weekStart: string
): { plannedMinutes: number; completedMinutes: number; deferredTasks: number } {
  const weekEnd = shiftDateKey(weekStart, 6);
  const plans = Object.values(state.dailyPlans ?? {}).filter(plan =>
    isDateKeyInRange(plan.date, weekStart, weekEnd)
  );
  return {
    plannedMinutes: plans.reduce(
      (sum, plan) =>
        sum +
        plan.tasks
          .filter(task => task.status !== "dismissed")
          .reduce((taskSum, task) => taskSum + task.minutes, 0),
      0
    ),
    completedMinutes: plans.reduce(
      (sum, plan) =>
        sum +
        plan.tasks
    ...
    deferredTasks: plans.reduce(
      (sum, plan) =>
        sum + plan.tasks.filter(task => task.status === "deferred").length,
      0
    ),
  };
}
...
export function nextRetrievalInterval(repetitions: number): number {
  return RETRIEVAL_INTERVALS[
    Math.min(Math.max(repetitions, 0), RETRIEVAL_INTERVALS.length - 1)
  ];
}

export function getDueRetrievalPrompts(
  state: MeridianStateSnapshot,
  date: string
): RetrievalPrompt[] {
...
export function getUpcomingRetrievalPrompts(
  state: MeridianStateSnapshot,
  date: string
): RetrievalPrompt[] {
  return (state.retrievalPrompts ?? [])
...
export function getLearningHeatmap(
  state: MeridianStateSnapshot,
  endDate: string,
  days = 28
): Array<{ date: string; minutes: number; touched: boolean }> {
  return Array.from({ length: days }, (_, index) => {
    const date = shiftDateKey(endDate, index - days + 1);
    const log = state.dailyLogs[date];
    return {
      date,
      minutes: getLearningMinutes(log),
      touched: Boolean(log?.touched),
    };
  });
}

export function getPhaseTimeBreakdown(
  model: MeridianModel,
  state: MeridianStateSnapshot
): Array<{ phaseId: string; label: string; minutes: number }> {
  const sessionToPhase = new Map(
    model.phases.flatMap(phase =>
      phase.sessions.map(session => [session.id, phase] as const)
    )
  );
  ...
  for (const log of Object.values(state.dailyLogs)) {
    for (const entry of log.timeEntries ?? []) {
      const phase = entry.sessionId
        ? sessionToPhase.get(entry.sessionId)
        : undefined;
      if (phase)
        minutes.set(
          phase.id,
          (minutes.get(phase.id) ?? 0) + Math.round(entry.seconds / 60)
        );
    }
  }
  return model.phases.map(phase => ({
    phaseId: phase.id,
    label: `Phase ${phase.identity.number + 1}`,
    minutes: minutes.get(phase.id) ?? 0,
  }));
}

export function getCapabilityTrajectory(
  model: MeridianModel,
  state: MeridianStateSnapshot
): Array<{ phaseId: string; label: string; percent: number }> {
  return model.phases.map(phase => ({
    phaseId: phase.id,
    label: `P${phase.identity.number + 1}`,
    percent: getPhaseProgress(model, phase, state).percent,
  }));
}
