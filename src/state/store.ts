...
export interface MeridianState extends MeridianStateSnapshot {
  ...
  completeSession: (
    id: string,
    entry: JournalEntry,
    evidence?: Omit<EvidenceRecord, "schemaVersion" | "createdAt" | "updatedAt">
  ) => void;
  ...
  setDailyLog: (
    ...
    minutes: number,
    note?: string,
    metrics?: Partial<Pick<DailyLog, "build" | "study" | "absorb" | "sleep">>
  ) => void;
  recordTime: (
    entry: Omit<TimeEntry, "id" | "createdAt"> &
      Partial<Pick<TimeEntry, "id" | "createdAt">>
  ) => void;
  ...
  saveSessionAttempt: (
    attempt: Omit<SessionAttempt, "schemaVersion" | "updatedAt">
  ) => void;
  ...
  updatePortfolio: (
    id: string,
    entry: Partial<{ kind: string; title: string; url: string; note: string }>
  ) => void;
...
export function getPersistedMeridianState(
  state: MeridianState
...
export function getInitialMeridianState() {
  ...
  persist(
    set => ({
      ...
      setScheduleMode: scheduleMode =>
        set(state => ({ settings: { ...state.settings, scheduleMode } })),
      toggleCheck: id =>
        set(state => ({
      ...
      setSessionStep: (id, currentStep) =>
        set(state => ({
          sessions: {
            ...
            [id]: {
              currentStep,
              completed: state.sessions[id]?.completed ?? false,
            },
          },
        })),
      startSession: id =>
        set(state => ({
          counters: {
            ...state.counters,
            sessionsStarted: state.counters.sessionsStarted + 1,
          },
          sessions: {
            ...state.sessions,
            [id]: {
              currentStep: state.sessions[id]?.currentStep ?? 0,
              completed: false,
            },
          },
        })),
        ...
        set(state => {
          ...
          return {
            sessions: {
              ...state.sessions,
              [id]: {
                ...(state.sessions[id] ?? { currentStep: 0 }),
                completed: true,
                completedAt: entry.date,
              },
            },
            sessionAttempts: Object.fromEntries(
              Object.entries(state.sessionAttempts ?? {}).filter(
                ([sessionId]) => sessionId !== id
              )
            ),
            journals: [
              ...state.journals.filter(item => item.id !== entry.id),
              entry,
            ],
            retrievalPrompts:
              entry.confusion?.trim() || entry.nextQuestion?.trim()
                ? (() => {
                    const question =
                      entry.nextQuestion?.trim() ||
                      `Can I now explain: ${entry.confusion?.trim()}?`;
            ...
            counters: {
              ...state.counters,
              sessionsCompleted:
                state.counters.sessionsCompleted +
                (state.sessions[id]?.completed ? 0 : 1),
            },
            dailyLogs: {
              ...state.dailyLogs,
              [entry.date]: {
      ...
      setHabit: (date, id, value) =>
        set(state => {
          const previous = state.dailyLogs[date] ?? {
            date,
            minutes: 0,
            touched: false,
          };
            ...
            dailyPlans: state.dailyPlans?.[date]
              ? {
                  ...
                  [date]: {
                    ...
                    tasks: state.dailyPlans[date].tasks.map(task =>
                      task.id === `habit:${id}`
                        ? {
                            ...task,
                            status: value
          ...
          return {
            dailyLogs: {
              ...
              [date]: {
                ...
                touched:
                  Boolean(previous.touched) ||
                  manualMinutes > 0 ||
                  Boolean(note) ||
                  Object.values(previous.habits ?? {}).some(Boolean),
                note,
                ...metrics,
              },
            },
      ...
      recordTime: entry =>
        set(state => {
          const previous = state.dailyLogs[entry.date] ?? {
            date: entry.date,
            minutes: 0,
            touched: false,
          };
          const normalized: TimeEntry = {
            id:
              entry.id ??
              `${entry.source}.${entry.sessionId ?? "general"}.${entry.date}.${Date.now()}`,
            createdAt: entry.createdAt ?? new Date().toISOString(),
            ...entry,
          };
          const timeEntries = [
            ...(previous.timeEntries ?? []).filter(
      ...
      setProgressionOverride: (key, reason) =>
        set(state => ({
          progressionOverrides: {
      ...
      clearProgressionOverride: key =>
        set(state => {
          const { [key]: _removed, ...progressionOverrides } =
            state.progressionOverrides ?? {};
          void _removed;
      ...
      setWeeklyDecisions: (week, decisions) =>
        set(state => ({
          reviews: {
            ...(state.reviews ?? {}),
            [week]: {
      ...
      createRetrievalPrompt: entry =>
        set(state => {
          const question =
            entry.nextQuestion?.trim() ||
            (entry.confusion?.trim()
              ? `Can I now explain: ${entry.confusion.trim()}?`
              : "");
          if (!question) return {};
          const now = new Date().toISOString();
        ...
        set(state => ({
          retrievalPrompts: (state.retrievalPrompts ?? []).map(prompt => {
            ...
            return {
              ...
              repetitions,
              intervalDays,
              dueDate: shiftDateKey(date, intervalDays),
      ...
      snoozeRetrievalPrompt: (id, date) =>
        set(state => ({
          retrievalPrompts: (state.retrievalPrompts ?? []).map(prompt =>
            prompt.id === id && prompt.status !== "mastered"
              ? {
                  ...prompt,
                  dueDate: shiftDateKey(date, 1),
      ...
      discardSessionAttempt: sessionId =>
        set(state => ({
          sessionAttempts: Object.fromEntries(
            Object.entries(state.sessionAttempts ?? {}).filter(
              ([id]) => id !== sessionId
      ...
      setPlanTaskStatus: (date, taskId, status, deferredTo) =>
        set(state => {
                ...
                tasks: plan.tasks.map(task =>
                  task.id === taskId
                    ? {
                        ...
                        status,
                        deferredTo:
                          status === "deferred" ? deferredTo : undefined,
                        completedAt:
                          status === "completed"
                            ? new Date().toISOString()
                            : undefined,
      ...
      updateEvidence: (id, evidence) =>
        set(state => ({
          evidence: (state.evidence ?? []).map(item =>
            item.id === id
              ? { ...item, ...evidence, updatedAt: new Date().toISOString() }
              : item
          ),
        })),
      removeEvidence: id =>
      ...
      setResourceState: (resourceId, status, note) =>
        set(state => {
          const now = new Date().toISOString();
          const previous = state.resourceStates?.[resourceId];
          const value: ResourceProgress = {
            status,
      ...
      addPortfolio: entry =>
        set(state => ({
          portfolio: { entries: [...(state.portfolio?.entries ?? []), entry] },
        })),
      updatePortfolio: (id, entry) =>
        set(state => ({
          portfolio: {
            entries: (state.portfolio?.entries ?? []).map(item =>
      ...
      removePortfolio: id =>
        set(state => ({
          portfolio: {
            entries: (state.portfolio?.entries ?? []).filter(
              item => item.id !== id
            ),
          },
        })),
    ...
    }),
    {
      name: "meridian.v1",
      version: 2,
      // Server-authoritative storage; browser persistence is only read for explicit legacy imports.
      ...
      migrate: persisted => ({
        ...initialState,
        ...(persisted as Partial<MeridianState>),
      }),
