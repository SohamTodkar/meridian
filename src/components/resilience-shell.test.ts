...
const runner = readFileSync(
  resolve(process.cwd(), "src/components/session-runner.tsx"),
  "utf8"
);
const modal = readFileSync(
  resolve(process.cwd(), "src/components/modal-portal.tsx"),
  "utf8"
);
const portfolio = readFileSync(
  resolve(process.cwd(), "src/components/stage3-views.tsx"),
  "utf8"
);
const offlineRuntime = readFileSync(
  resolve(process.cwd(), "src/components/offline-runtime.tsx"),
  "utf8"
);
...
describe("resilient accessible shell", () => {
  ...
  it("renders an explicit resume-or-discard choice for a recovered session attempt", () => {
    expect(runner).toContain("Recovered session attempt");
  ...
  it("retires stale offline shells and names the recovered-session decision group", () => {
    expect(offlineRuntime).toContain("registration.unregister()");
    expect(offlineRuntime).not.toContain("serviceWorker.register(");
