...
function PageFrame({ eyebrow, title, intro, children }: { eyebrow: string; title: string; intro?: string; children: React.ReactNode }) {
  return <div className="content"><div className="page-kicker eyebrow">{eyebrow}</div><h1 className="page-title">{title}</h1>{intro && <p className="page-intro">{intro}</p>}{children}</div>;
}
...
export function RecallView() {
  ...
  return <PageFrame eyebrow="Recall · spaced practice" title="Answer before you look." intro="Confusions and next questions become small, private retrieval prompts. Recall the idea in your own words, then check the original entry or resource.">
...
export function ReviewView() {
  ...
  const sessionMinutes = logs.flatMap(log => log.timeEntries ?? []).filter(entry => entry.source === "session").reduce((sum, entry) => sum + entry.seconds / 60, 0);
  const focusMinutes = logs.flatMap(log => log.timeEntries ?? []).filter(entry => entry.source === "focus").reduce((sum, entry) => sum + entry.seconds / 60, 0);
  const sessionProofs = new Set(state.journals.filter(entry => entry.sessionId && entry.date >= week && entry.date <= weekEnd).map(entry => entry.sessionId)).size;
  ...
  return <PageFrame eyebrow={`Weekly review · week of ${week}`} title="Look back honestly." intro="The weekly review is what catches drift before it becomes a fade. Do it even in a week you broke the streak — especially then.">
    <div className="metric-grid review-stats">
      <div className="metric"><span className="eyebrow">Guided sessions</span><strong className="metric-value">{(sessionMinutes / 60).toFixed(1)}h</strong></div>
      <div className="metric"><span className="eyebrow">Focus blocks</span><strong className="metric-value">{(focusMinutes / 60).toFixed(1)}h</strong></div>
      <div className="metric"><span className="eyebrow">Other study</span><strong className="metric-value">{(Math.max(0, totals.minutes - sessionMinutes - focusMinutes) / 60).toFixed(1)}h</strong></div>
...
export function PortfolioView() {
  ...
  const add = () => {
    setCaptureError("");
    if (!draft.title.trim()) { setCaptureError("Give this evidence a title first."); return; }
    if (draft.url.trim()) {
      try { const link = new URL(draft.url.trim()); if (!["https:", "http:"].includes(link.protocol)) throw new Error(); }
      catch { setCaptureError("Use a complete link beginning with https:// or http://."); return; }
    }
  ...
  return <PageFrame eyebrow="Evidence vault" title="Keep proof attached to the learning." intro="Verified session evidence, project artifacts, notes, and links stay connected to your learning record.">
    <section id="capture" className="reference-card evidence-capture"><div className="eyebrow">{editingId ? "Edit evidence" : "Capture evidence"}</div><div className="form-grid"><select aria-label="Evidence kind" value={draft.kind} onChange={(e) => setDraft({ ...draft, kind: e.target.value as typeof draft.kind })}><option value="artifact">Artifact</option><option value="link">Link</option><option value="note">Note</option><option value="session-proof">Session proof</option></select><select aria-label="Evidence status" value={draft.proofStatus} onChange={(e) => setDraft({ ...draft, proofStatus: e.target.value as typeof draft.proofStatus })}><option value="captured">Captured</option><option value="verified">Verified</option><option value="planned">Planned</option></select><input ref={captureInput} aria-label="Evidence title" placeholder="Evidence title" value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} /><input aria-label="Capability or topic" placeholder="Capability or topic" value={draft.capability} onChange={(e) => setDraft({ ...draft, capability: e.target.value })} /><input aria-label="Evidence URL" placeholder="URL (optional)" value={draft.url} onChange={(e) => setDraft({ ...draft, url: e.target.value })} /><textarea aria-label="Evidence note" className="journal-input" placeholder="What does this prove?" value={draft.note} onChange={(e) => setDraft({ ...draft, note: e.target.value })} /></div><div className="runner-controls"><button className="button-primary" type="button" onClick={add}><Plus size={15} /> {editingId ? "Save evidence" : "Capture evidence"}</button>{editingId && <button className="button-secondary" type="button" onClick={() => { setDraft(emptyDraft); setEditingId(undefined); }}>Cancel</button>}</div></section>
    {captureError && <p className="form-error" role="alert">{captureError}</p>}
    {undoRecord && <section className="evidence-recovery" role="status"><span><strong>Evidence removed.</strong> The original journal record was left intact.</span><div className="runner-controls"><button className="button-secondary" type="button" onClick={undoDelete}>Undo removal</button><button className="quiet-link" type="button" onClick={() => setUndoRecord(undefined)}>Dismiss</button></div></section>}
    <div className="filter-row"><label className="eyebrow" htmlFor="evidence-filter">Show</label><select id="evidence-filter" value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">All evidence</option><option value="session-proof">Verified session proofs</option><option value="artifact">Artifacts</option><option value="link">Links</option><option value="note">Notes</option></select><span className="muted">{visibleEvidence.length} saved record{visibleEvidence.length === 1 ? "" : "s"}</span></div>
    <div className="journal-list">{visibleEvidence.length ? visibleEvidence.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map((entry) => <article className="journal-entry evidence-entry" key={entry.id}><div className="eyebrow">{entry.proofStatus} · {entry.kind.replace("-", " ")} · {new Date(entry.updatedAt).toLocaleDateString()}</div><h2 className="section-title">{entry.title}</h2>{entry.capability && <p className="evidence-capability">Capability · {entry.capability}</p>}<p>{entry.note}</p>{entry.url && <a className="quiet-link" href={entry.url} target="_blank" rel="noreferrer">{entry.url}</a>}<div className="evidence-actions"><button className="icon-button" type="button" aria-label={`Edit ${entry.title}`} onClick={() => { setEditingId(entry.id); setDraft({ kind: entry.kind === "session-proof" ? "session-proof" : entry.kind, title: entry.title, url: entry.url ?? "", note: entry.note, capability: entry.capability ?? "", proofStatus: entry.proofStatus }); }}><Pencil size={14} /></button><button className="icon-button" type="button" aria-label={`Remove ${entry.title}`} onClick={() => { setPendingDelete(entry); setUndoRecord(undefined); }}><Trash2 size={14} /></button></div>{pendingDelete?.id === entry.id && <div className="evidence-delete-confirm" role="alert"><p>Remove this evidence record from your workspace? Its original journal entry will stay intact.</p><div className="runner-controls"><button className="button-secondary" type="button" onClick={() => setPendingDelete(undefined)}>Keep evidence</button><button className="button-danger" type="button" onClick={confirmDelete}>Remove evidence</button></div></div>}</article>) : <div className="empty-state">Complete a verified session or capture an artifact, note, or link here.</div>}</div>
