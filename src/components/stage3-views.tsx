...
function PageFrame({ eyebrow, title, intro, children }: { eyebrow: string; title: string; intro?: string; children: React.ReactNode }) {
  return <div className="content"><div className="page-kicker eyebrow">{eyebrow}</div><h1 className="page-title"><KineticTypography as="span" variant="wave">{title}</KineticTypography></h1>{intro && <p className="page-intro">{intro}</p>}{children}</div>;
}
...
export function DsaView() {
  ...
  return <PageFrame eyebrow={`DSA track · ${progress.percent}%`} title="Keep the fundamentals close." intro={dsa.purpose}>
    ...
    <section className="reference-card warning-card"><div className="eyebrow">The trap</div><p>{dsa.warning}</p></section>
...
export function LibraryView() {
  ...
  return <PageFrame eyebrow="Library & network" title="Use the right resource at the right time." intro="A phase-first resource map keeps the core course visible and every useful reference in its proper place.">
    ...
    {activeTab !== "roadmap" && <section className="reference-card library-tab"><div className="eyebrow">{tab(activeTab)}</div>
      ...
      {activeTab === "books" && <>{l.books.map((x: any, index: number) => <LabeledItem key={`${x.title}-${index}`} name={x.title} qualifier={`${x.author} · ${x.rating}/5`} meta={x.when}>{x.note}</LabeledItem>)}{l.budget.map((x: any, index: number) => <LabeledItem key={`${x.item}-${index}`} name={x.item} qualifier={x.worth} meta={`${x.cost} · ${x.when}`} />)}</>}
      {activeTab === "university" && <>{l.university.years.map((x: any, index: number) => <LabeledItem key={`${x.year}-${index}`} name={x.year} qualifier={x.focus}>{x.why}</LabeledItem>)}{l.university.plays.map((x: any, index: number) => <LabeledItem key={`${x.key}-${index}`} name={x.text}>{x.detail}</LabeledItem>)}</>}
    </section>}
