...
import type { CurriculumSection } from "@/data";
import { model } from "@/data";
import { checkpointEvidence } from "@/lib/checkpoint";
import {
  getPhaseAccess,
  getPhaseProgress,
  getSessionAccess,
  ...
  return (
    <div className="curriculum-item">
      <TickBox
        ...
        checked={checked}
        label={`Mark ${text} ${checked ? "not done" : "done"}`}
        onChange={onChange}
      />
      <DisplayPair
        className="curriculum-item-copy"
        title={text}
        meta={meta || undefined}
      />
    </div>
...
function CurriculumSectionView({
  section,
  checked,
  onChange,
}: {
  ...
  return (
    <article className="curriculum-section">
      <div className="curriculum-section-heading">
        <div>
          {section.title && (
            <h3 className="curriculum-title">{section.title}</h3>
          )}
          {section.note && <p className="curriculum-note">{section.note}</p>}
        </div>
        <span className="eyebrow">
          {section.kind === "checklist"
            ? "Checklist"
            : section.kind === "checktable"
              ? "Table"
              : section.kind === "days"
                ? "Days"
                : section.kind === "videos"
...
function PhaseHubContent({ phaseId }: { phaseId: string }) {
  ...
  const tab: Tab = allowed.includes(candidate as Tab)
    ? (candidate as Tab)
    : "overview";
  ...
  const evidence = useMemo(
    () => (phase ? checkpointEvidence(phase) : []),
    [phase]
  );
  if (!phase || !progress)
    return (
      <div className="content">
        <div className="empty-state">This phase does not exist.</div>
      </div>
    );
  ...
  if (!access.allowed)
    return (
      <ProgressionLock
        ...
        overrideKey={phaseOverrideKey(phase.id)}
      />
    );
  const cleared = isPhaseCleared(phase, state);
  const phaseOverride =
    state.progressionOverrides?.[phaseOverrideKey(phase.id)];
  const tabs: Tab[] = [
    "overview",
    "sessions",
    "curriculum",
  ...
  return (
    <div className="content">
      <div className="page-kicker eyebrow">
        Phase {phase.identity.number + 1} · {progress.percent}% weighted
        capability · {progress.coverage.percent}% curriculum coverage
      </div>
      <h1 className="page-title">{phase.identity.northstarName}</h1>
      ...
      {phaseOverride && (
        <aside className="override-notice">
          ...
          <span>
            Opened {new Date(phaseOverride.createdAt).toLocaleDateString()} ·{" "}
            {phaseOverride.reason}
          </span>
        </aside>
      )}
      <nav className="phase-tabs" aria-label="Phase sections">
        {tabs.map(item => (
          <Link
            className={`phase-tab ${tab === item ? "active" : ""}`}
            href={`/path/${phase.id}?tab=${item}`}
            key={item}
          >
            {item}
          </Link>
      ...
      {tab === "overview" && (
        <div className="overview-grid">
          ...
          <div className="info-block">
            ...
            <p>{phase.identity.primary}</p>
          </div>
          <div className="info-block">
            <div className="eyebrow">Duration</div>
            <p>{phase.identity.typicalRange}</p>
            <p>
              {phase.identity.weeks}{" "}
              <span className="muted">
                suggested calendar window · not a deadline
              </span>
            </p>
          </div>
          <div className="info-block">
            <div className="eyebrow">Objective</div>
            <p>{phase.identity.objective}</p>
          </div>
          <div className="info-block">
            <div className="eyebrow">Priorities</div>
            <ul>
              {phase.priorities.map(item => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
          <div className="info-block">
            <div className="eyebrow">Stretch</div>
            <ul>
              {phase.stretch.map(item => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </div>
      ...
      {tab === "sessions" && (
        <div className="session-list">
          {phase.sessions.map((session, index) => {
            ...
            return (
              <Link
                className={`session-row${sessionAccess.allowed ? "" : " locked"}`}
                href={`/session/${session.id}`}
                key={session.id}
                aria-label={`${sessionAccess.allowed ? "Open" : "View prerequisite for"} session ${index + 1}: ${session.title}`}
              >
                {row}
              </Link>
            );
        ...
        <div className="curriculum-group">
          {phase.curriculum.map(section => (
            <CurriculumSectionView
              ...
              section={section}
              checked={index =>
                Boolean(state.checks[`${section.key}.${index + 1}`])
              }
          ...
          {phase.resources.map(resource => (
            <article className="resource-row" key={resource.id}>
              <div>
                ...
                <p className="row-copy">{resource.role ?? resource.why}</p>
                {resource.note && <p className="row-copy">{resource.note}</p>}
                {resource.checked && (
                  <p className="resource-footnote">{resource.checked}</p>
                )}
              </div>
        ...
        <div className="section" style={{ marginTop: 28 }}>
          <div className="action-panel" style={{ marginTop: 0 }}>
            ...
            <h2
              ...
              style={{ fontSize: "clamp(25px,4vw,39px)" }}
            >
              {phase.identity.promise}
            </h2>
          ...
          <div className="eyebrow" style={{ marginTop: 42 }}>
            Evidence ·{" "}
            {
              evidence.filter(item => item.ids.every(id => state.checks[id]))
                .length
            }
            /{evidence.length}
          </div>
          <div className="evidence-list">
            {evidence.map(item => (
              <label className="evidence-row" key={item.id}>
                <TickBox
                  className="evidence-check"
                  checked={item.ids.every(id => Boolean(state.checks[id]))}
                  label={`Evidence: ${item.text}`}
                  onChange={() => {
                    const value = !item.ids.every(id => state.checks[id]);
                    for (const id of item.ids)
                      if (Boolean(state.checks[id]) !== value) toggleCheck(id);
                  }}
                />
                <span
                  className="row-copy"
                  style={{ color: "var(--bone-dim)", margin: 0 }}
                >
                  {item.text}
                </span>
                <span className="row-meta">
                  {item.ids.every(id => state.checks[id]) ? "Done" : "Open"}
                </span>
              </label>
            ))}
          </div>
          <div className="stop-note">
            <strong>{phase.checkpoint.cockpit.gate}</strong>
            <br />
            {phase.checkpoint.cockpit.proof}
          </div>
          {cleared && (
            <p className="dim" style={{ marginTop: 20 }}>
              <Check size={15} style={{ verticalAlign: "middle" }} /> Capability
...
export function PhaseHub({ phaseId }: { phaseId: string }) {
  return (
    <Suspense
      fallback={
        <div className="content">
          <div className="empty-state">Loading phase…</div>
        </div>
      }
    >
      <PhaseHubContent phaseId={phaseId} />
    </Suspense>
  );
}
